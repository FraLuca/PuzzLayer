...
from graph_construct.constants import NODE_TYPES, EDGE_TYPES

def make_node_feat(neuron_num, l, node_type, end_neuron=False):
    ''' end_neuron means whether it is an input or output neuron
    x has 3 dimensions:
    x[:, 0] is layer number
    x[:, 1] is neuron order numbers (if the neuron is an input or output neuron of the whole network, else -1)
    x[:, 2] is the node type (see constants.NODE_TYPES)
    '''
    x = torch.zeros(neuron_num, 3, dtype=torch.long)
    x[:, 0] = l
    if not end_neuron:
        # if it is a hidden neuron, don't give an order number
        x[:, 1] = -1
    else:
