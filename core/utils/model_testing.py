...
def test_on_mnist(reco_model, orig_model, filenames, reco_model_fromtext=None):
    ...
    reco_fromtext_accuracies_sum = 0

    for i in range(len(reco_model)):
        orig_model[i].eval()
        reco_model[i].eval()
        if reco_model_fromtext is not None:
            reco_model_fromtext[i].eval()
        ...
        for data, target in test_loader:
            ...
            if reco_model_fromtext is not None:
                output = reco_model_fromtext[i](data)
                reco_fromtext_correct += output.argmax(dim=1).eq(target).sum().item()
        
        orig_accuracies_sum += orig_correct/len(mnist_subset)
        reco_accuracies_sum += reco_correct/len(mnist_subset)
        reco_fromtext_accuracies_sum += reco_fromtext_correct/len(mnist_subset)

    avg_orig_accuracy = orig_accuracies_sum / len(orig_model)
    avg_reco_accuracy = reco_accuracies_sum / len(reco_model)
    avg_reco_fromtext_accuracy = reco_fromtext_accuracies_sum / len(reco_model)

    if reco_model_fromtext is not None:
        return avg_orig_accuracy, avg_reco_accuracy, avg_reco_fromtext_accuracy
    else:
        return avg_orig_accuracy, avg_reco_accuracy
